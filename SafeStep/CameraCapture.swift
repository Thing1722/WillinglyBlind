...
/// view when ARKit LiDAR is missing. You do not install ARKit — it ships
/// with iOS. What many phones lack is the LiDAR *hardware*.
...
extension CameraCapture: AVCaptureDepthDataOutputDelegate {
    func depthDataOutput(
        ...
        guard let map = DepthMap(pixelBuffer: converted.depthDataMap, confidence: nil) else { return }
        onDepth?(map)
    }
}
