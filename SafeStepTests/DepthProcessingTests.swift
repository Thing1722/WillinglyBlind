...
final class DepthProcessingTests: XCTestCase {
    func testUniformFrameFillsEveryCell() {
        ...
        for row in 0..<3 {
            for column in 0..<3 {
                ...
                XCTAssertEqual(cell.medianMeters ?? -1, 2.5, accuracy: 0.0001)
    ...
    func testEmptyFrameYieldsEmptyCells() {
        ...
        for cell in grid.cells {
            ...
            XCTAssertNil(cell.medianMeters)
    ...
    func testPixelSizeMatchesWidthAndHeight() {
        ...
        XCTAssertEqual(frame.sample(row: 3, column: 7), 1)
    }

    func testLowConfidencePixelsBecomeInvalid() {
        let meters: [Float] = [1.2, 2.4, 3.6, 4.8]
        let confidence: [UInt8] = [2, 0, 1, 0]
        let copied = DepthBufferCopy.copyMeters(
            width: 2,
            height: 2,
            meters: meters,
            confidence: confidence
        )
        XCTAssertEqual(copied[0], 1.2, accuracy: 0.0001)
        XCTAssertEqual(copied[1], 0)
        XCTAssertEqual(copied[2], 3.6, accuracy: 0.0001)
        XCTAssertEqual(copied[3], 0)

        let frame = DepthFrame.copying(width: 2, height: 2, meters: meters, confidence: confidence)
        XCTAssertFalse(DepthFrame.isValid(frame.meters[1]))
    ...
    func testCopyWithoutConfidenceKeepsFinitePositiveSamples() {
        ...
        XCTAssertEqual(copied[0], 0.4, accuracy: 0.0001)
        XCTAssertEqual(copied[1], 1.1, accuracy: 0.0001)
        XCTAssertTrue(copied[2].isNaN)
        XCTAssertEqual(copied[3], 2.0, accuracy: 0.0001)
    }

    func testCellMedianAndInvalidRatioSupportDropOff() {
        let meters: [Float] = [
            2.0, 2.0, 2.0,
            1.8, 1.9, 2.1,
            3.0, 0, 3.2
        ...
        let grid = DepthGridSampler.sample(DepthFrame(width: 3, height: 3, meters: meters))
        let mid = grid.cell(row: .mid, column: .center)
        let ground = grid.cell(row: .nearGround, column: .center)

        XCTAssertEqual(mid.medianMeters ?? -1, 1.9, accuracy: 0.0001)
        XCTAssertEqual(ground.validFraction, 0, accuracy: 0.0001)
        XCTAssertEqual(ground.invalidSampleRatio, 1.0, accuracy: 0.0001)
        XCTAssertTrue(
            DetectionConfig.standard.isDropOff(
                measuredGroundMeters: ground.medianMeters.map(Double.init),
                expectedWalkingSurfaceMeters: Double(mid.medianMeters ?? 0),
